...
function starting() {
    ...
    setInterval(function() {
        ...
        if (ObstacleTLeft < 75 && ObstacleTLeft > 73 || ObstacleT2Left < 75 && ObstacleT2Left > 73) {
            score += 0.5; //TODO meglio assolutamente
            scoreElem.innerHTML = score;
        }
        if ((ObstacleTLeft && ObstacleBLeft) < 100 && (ObstacleTLeft && ObstacleBLeft) > 50) {
            if (charY < ObstacleBTop || charY > 530 - ObstacleTBottom) alert("Game Over!");
        }
        if ((ObstacleT2Left && ObstacleB2Left) < 100 && (ObstacleT2Left && ObstacleB2Left) > 50) {
            if (charY < ObstacleB2Top || charY > 530 - ObstacleT2Bottom) alert("Game Over!");
        }
    }, 10);
...
function autoCorrelate(buf1, sampleRate) {
    ...
    for(var i1 = 0; i1 < SIZE / 2; i1++)if (Math.abs(buf1[i1]) < thres) {
        r1 = i1;
        break;
    }
    for(var i1 = 1; i1 < SIZE / 2; i1++)if (Math.abs(buf1[SIZE - i1]) < thres) {
        r2 = SIZE - i1;
        break;
    }
    buf1 = buf1.slice(r1, r2);
    SIZE = buf1.length;
    let c = new Array(SIZE).fill(0);
    for(var i1 = 0; i1 < SIZE; i1++)for(let j = 0; j < SIZE - i1; j++)c[i1] = c[i1] + buf1[j] * buf1[j + i1];
    let d = 0;
    while(c[d] > c[d + 1])d++;
    let maxval = -1, maxpos = -1;
    for(let i2 = d; i2 < SIZE; i2++)if (c[i2] > maxval) {
        maxval = c[i2];
        maxpos = i2;
    }
    let T0 = maxpos;
...
//# sourceMappingURL=index.7048523e.js.map
